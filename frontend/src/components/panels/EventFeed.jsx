    ...
    <div className="panel flex h-full flex-col p-5">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-slate-400">Live network event stream</div>
        </div>
      ...
      <div className="scrollbar-thin space-y-3 overflow-y-auto pr-2">
        {events.map((event, index) => (
          <motion.div
            ...
            className="rounded-2xl border border-line/70 bg-white/[0.02] p-4"
          >
            <div className="mb-2 flex items-start justify-between gap-3">
              <div>
                <div className="font-semibold text-white">{event.title}</div>
                <div className="text-xs tracking-[0.2em] text-slate-500">{event.time}</div>
              </div>
            ...
            <div className="text-sm text-slate-400">{event.amount}</div>
          </motion.div>
        ))}
      </div>
    </div>
