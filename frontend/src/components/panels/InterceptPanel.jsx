    ...
    <div className="panel p-5">
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <div className="text-sm text-white/40">Cross-bank freeze orchestration simulation</div>
        </div>
        <div className="rounded-full border border-[#FF4500]/30 bg-[#FF4500]/10 px-3 py-1 text-xs uppercase tracking-[0.2em] text-[#FF4500]">
          {intercept.freeze_coverage_ratio}% funds covered
        </div>
      </div>
      <div className="grid gap-3 lg:grid-cols-3">
        {intercept.frozen_accounts.map((action) => (
          <div key={action.account_id} className="rounded-2xl border border-white/[0.07] bg-white/[0.02] p-4">
            <div className="mb-1 font-semibold text-white">{action.account_id}</div>
            <div className="text-sm text-white/40">{action.bank}</div>
            <div className="mt-3 font-display text-2xl text-[#FF4500]">{formatCurrency(action.amount_frozen)}</div>
            <div className="mt-2 flex items-center justify-between text-xs uppercase tracking-[0.18em] text-white/30">
      ...
      <div className="mt-4 rounded-3xl border border-orange/30 bg-orange/10 p-4">
        ...
        <div className="text-sm text-white/50">{intercept.simulate_3_minute_delay.narrative}</div>
